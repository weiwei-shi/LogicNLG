...
if __name__ == '__main__':
    ...
    if args.do_train:
        ...
        dataset = GPTTableDatabase('data/train_lm2.json', None, None, tokenizer, args.batch_size, args.max_len)        
        model.train()
        ...
        for epoch_idx in range(args.epoch):
            ...
            if args.model == 'gpt2':
                torch.save(model.state_dict(), '{}/GPT_type_ep{}.pt'.format(args.id, epoch_idx))
            else:
                torch.save(model.state_dict(), '{}/GPT_type_medium_ep{}.pt'.format(args.id, epoch_idx))
        tb_writer.close()
    ...
    if args.do_test:
        dataset = GPTTableDatabase(None, None, 'data/test_lm3.json', tokenizer, args.batch_size, args.max_len)
        model.load_state_dict(torch.load(args.load_from))
        model.eval()
    ...
    if args.do_verify:
        dataset = GPTTableDatabase(None, None, 'data/test_lm_pos_neg2.json', tokenizer, args.batch_size, args.max_len)
        model.load_state_dict(torch.load(args.load_from))
        model.eval()
        correct, total = 0, 0
        with torch.no_grad():
