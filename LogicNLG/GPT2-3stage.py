...
if __name__ == '__main__':
    ...
    parser.add_argument('--stage', default=0, type=int, help="whether to train or test the model")
    ...
    if args.do_train:
        ...
        tb_writer = SummaryWriter(log_dir='tensorboard/GPT_stage{}_3S/{}'.format(args.stage, recording_time))        
        dataset = GPTTableCoarseFineDatabase2('data/train_lm_preprocessed3.json', None, None,
        ...
        for epoch_idx in range(args.start_epoch, args.start_epoch + args.epoch):
            ...
            if args.model == 'gpt2':
                torch.save(model.state_dict(), '{}/GPT_stage{}_3S_type_ep{}.pt'.format(args.id, args.stage, epoch_idx))
            else:
                torch.save(model.state_dict(), '{}/GPT_stage{}_3S_medium_ep{}.pt'.format(args.id, args.stage, epoch_idx))

    if args.do_test:
        assert 'stage2' in args.load_from, "The testing can only be done with stage2 model"
        dataset = GPTTableCoarseFineDatabase2(None, None, 'data/test_lm3.json', tokenizer, args.batch_size, args.max_len, args.stage)
        model.load_state_dict(torch.load(args.load_from))
        model.eval()
        ...
        with open('outputs/GPT_{}_3S_type_{}.json'.format(args.model, bleu_3), 'w') as f:
            json.dump(results, f, indent=2)

    if args.do_verify:
        assert 'stage2' in args.load_from, "The testing can only be done with stage2 model"
        assert args.stage == 2, "The verification can only be done with stage 2 model"
        dataset = GPTTableCoarseFineDatabase2(None, None, 'data/test_lm_pos_neg2.json', 
                                             tokenizer, args.batch_size, args.max_len, args.stage)
