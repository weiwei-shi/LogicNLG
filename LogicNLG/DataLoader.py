...
class Dataloader(object):
    ...
    def shuffle(self):
        random.shuffle(self.train_ids) # 将序列中的元素随机打乱
...
class GPTTableDatabase(Dataloader):
    ...
    def get_data(self, idx, option='train', details=False):
        ...
        d = pandas.read_csv('data/all_csv/' + table_id, '#') # 得到对应的表格
        ...
        if option in ['train']:
            ...
            entry = entry[:self.batch_size] # 最多保留五个
        # 因为相关列不同，所以每个句子都要进行一次对表的线性化
        for e in entry:
            ...
            for i in range(len(d)):
                ...
                for _ in e[1]: #相关列
                    if isinstance(d.iloc[i][columns[_]], str): # 判断类型
            ...
            tmp_type = self.tokenizer.tokenize('Given the logic type : "{}" . '.format(e[4]))
            tmp_suffix = self.tokenizer.tokenize('Start describing : ')
            # 最终线性化结果：表名+表内容
            descs.append(self.tokenizer.convert_tokens_to_ids(tmp_prefix + tmp_idx + tmp_type + tmp_suffix))

        length = max([len(_) for _ in seqs]) + 1

        for i in range(len(seqs)):
            seqs[i] += (length - len(seqs[i])) * [self.tokenizer.eos_token_id] # 将句子的长度补充到一样
...
class GPTTableCoarseFineDatabase2(Dataloader):
    def __init__(self, train_name, val_name, test_name, tokenizer, batch_size=5, max_len=800, stage=0, total_stage=2): # 注意要修改
    ...
    def get_train_data(self, idx):
        ...
        seq_masks = []
        for e in entries:
            if self.stage == 0: # output为表面事实
                # if e[5] == "No":
                #     continue
                # else: 
                seqs.append(self.tokenizer.encode(e[5], add_special_tokens=False))
                seq_masks.append([1] * len(seqs[-1]))
            elif self.stage == 1: # output为粗模板
                seqs.append(self.tokenizer.encode(e[3], add_special_tokens=False))
                seq_masks.append([1] * len(seqs[-1]))
                # part1 = self.tokenizer.encode(e[5] + ' [SEP] ', add_special_tokens=False)
                # part2 = self.tokenizer.encode(e[3], add_special_tokens=False)
                # seqs.append(part1 + part2)
                # seq_masks.append([1] * len(part1) + [1] * len(part2))
            elif self.stage == 2: # output为粗模板+最终句子
                part1 = self.tokenizer.encode(e[3] + ' [SEP] ', add_special_tokens=False)
                part2 = self.tokenizer.encode(e[0], add_special_tokens=False)
                seqs.append(part1 + part2)
                seq_masks.append([1] * len(part1) + [1] * len(part2))
            ...
            tmp_prefix = self.tokenizer.tokenize('Given the table title of "{}" . '.format(e[2]))
            tmp_type = self.tokenizer.tokenize('Given the logic type : "{}" . '.format(e[6]))

            descs.append(self.tokenizer.convert_tokens_to_ids(tmp_prefix + tmp_idx + tmp_type))
            # descs.append(self.tokenizer.convert_tokens_to_ids(tmp_prefix + tmp_idx))

        length = max([len(_) for _ in seqs]) + 1

        for i in range(len(seqs)):
            seqs[i] += (length - len(seqs[i])) * [self.tokenizer.eos_token_id] # 补齐
            seq_masks[i] = seq_masks[i] + [1] + (length - len(seq_masks[i]) - 1) * [0] 
        seqs = torch.LongTensor(seqs)
        seq_masks = torch.FloatTensor(seq_masks)
    ...
    def get_data(self, idx, option, details=False):
        ...
        for e in entry:
            if self.stage == 0:
                ...
                seqs.append(self.tokenizer.encode(e[3], add_special_tokens=False))
                seq_masks.append([1] * len(seqs[-1]))
                # part1 = self.tokenizer.encode(e[4] + ' [SEP] ', add_special_tokens=False)
                # part2 = self.tokenizer.encode(e[3], add_special_tokens=False)
                # seqs.append(part1 + part2)
                # seq_masks.append([1] * len(part1) + [1] * len(part2))
            elif self.stage == 2:
    ...
    def get_pair_data(self, idx, option='val', mask_type='both'):
        ...
        for opt in ['pos', 'neg']:
            ...
            for e in entry:
                ...
                descs.append(self.tokenizer.convert_tokens_to_ids(tmp_idx + tmp_type))
                # descs.append(self.tokenizer.convert_tokens_to_ids(tmp_idx))
